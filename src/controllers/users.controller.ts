...
export const getUserMovies = async (req: express.Request, res: express.Response) => {
	try {
		...
		const user = await prisma.user.findUnique({ where: { username }, select:{movies:true}})
		if (!user) {
			return res.status(404).json({ message: 'User not found' })
		}
		return res.status(200).json(user)
...
export const getUserMoviesById = async (req: express.Request, res: express.Response) => {
	try {
		...
		const user = await prisma.user.findUnique({ where: { id: parseInt(req.params.id) }, select:{movies:true}})
		if (!user) {
			return res.status(404).json({ message: 'User not found' })
		}
		return res.status(200).json(user)
